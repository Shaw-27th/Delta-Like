...
public partial class ExtractionDemo : Node2D
{
    ...
    private readonly Rect2 _extract = new(new Vector2(945, 535), new Vector2(110, 70));
    ...
    private void UpdateFight(Unit u, float dt)
    {
        ...
        if (distance > u.Range * 0.85f) MoveUnit(u, e.Position, dt, 1f);
        else if (cover != Vector2.Zero) MoveUnit(u, cover, dt, 0.9f);
        else if (distance < 100f) MoveUnit(u, u.Position - u.Facing * 55f, dt, 0.85f);
        if (u.Cooldown <= 0f && u.Ammo > 0 && CanSee(u.Position, e.Position, u.Range))
        {
            ...
            e.Hp -= isMarksman ? 2 : 1;
            _fx.Add(new Fx { From = u.Position, To = e.Position, Color = new Color(0.4f, 0.95f, 1f), TimeLeft = 0.12f });
    ...
    private void MoveUnit(Unit u, Vector2 target, float dt, float factor)
    {
        ...
        u.Position = ResolveMove(u.Position, u.Radius, u.Position + u.Facing * u.Speed * factor * dt);
    }
